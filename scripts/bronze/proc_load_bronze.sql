/*****************************************************************************************
   ...
   Script Purpose   : This stored procedure loads data into the 'bronze' schema 
                      from external CSV files. It performs the following actions:
                        ...
                        - Uses the 'BULK INSERT' command to load data from CSV 
                          files into bronze tables.

   Author      : Jitendra Kumar Manasingh
   Created On  : 23/08/2025

   Description :
       - Truncates Bronze tables (clears old data).
       - Bulk inserts fresh CSV data from CRM and ERP source files.
       - Prints progress messages and logs duration for each load step.
       - Provides basic error handling for debugging purposes.
   ...
   WARNING :
       ...
       - BULK INSERT requires that the specified file paths exist 
         and SQL Server has read access.
       - Ensure proper permissions on the data folder before running.
*****************************************************************************************/
    ...
    BEGIN TRY
        ...
        *****************************************************************************************/
        ...
        WITH (
            ...
            ROWTERMINATOR = '\n',      -- Row terminator
            TABLOCK                    -- Use table-level lock for faster load
        );
